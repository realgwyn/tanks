package org.game.tanks.demo.graphics;
...
public class RotationPanel extends JPanel {
  ...
  public void rotatePointMatrix(Point[] origPoints, double angle, Point[] storeTo) {
    ...
    /*
     * We ge the original points of the polygon we wish to rotate and rotate them with affine transform to the given
     * angle. After the opeariont is complete the points are stored to the array given to the method.
     */
  ...
  public Point[] getOriginalPoints(int type) {
    ...
    /*
     * In this example the rotated "polygon" are stored in this method. The Point is that if we want to rotate a polygon
     * constatnly/frequently we cannot use the values of an already rotated polygon as this will lead to the polygon
     * deforming severely after few translations due to the points being constantly rounded. So the trick is to save the
     * original Points of the polygon and always rotate that one to the new angle instead of rotating the same one again
     * and again.
     */
